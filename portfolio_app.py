...
/* ===================== CLIENT-SIDE ROUTER CSS ===================== */
/* Hide Streamlit Header */
header[data-testid="stHeader"] { display: none !important; }

/* All 3 page containers start absolutely positioned and hidden */
div[data-testid="stVerticalBlock"]:has(> div.element-container .page-marker) {
...
/* The active page class applied by JS smoothly brings it into view */
div[data-testid="stVerticalBlock"].page-active {
...
# ====================== LIVE PRICE FUNCTION ======================
# Increased caching time here since the JS script handles live reloading automatically
...
with home_container:
    st.markdown('<div id="marker-home" class="page-marker"></div>', unsafe_allow_html=True)
                    ...
                    cards.forEach(card => {{
                        ...
                        if (data.RAW && data.RAW[sym] && data.RAW[sym].USD) {{
                            ...
                            if (newPrice !== undefined) {{
                                ...
                                // Update Historical Metrics directly via math
...
# =========================================================================
# ====================== 2. RENDER CRYPTO PAGE ============================
# =========================================================================
crypto_container = st.container()
with crypto_container:
    st.markdown('<div id="marker-crypto" class="page-marker"></div>', unsafe_allow_html=True)
    ...
    div[data-testid="stForm"]:has(.add-tx-card) input { background: rgba(255,255,255,0.03) !important; border: 1px solid rgba(255,255,255,0.1) !important; color: #fff !important; border-radius: 8px !important; }
    ...
    div[data-testid="stForm"]:has(.add-tx-card) div[data-testid="stHorizontalBlock"]:has(> div[data-testid="column"]:nth-child(2):last-child) > div:nth-child(1) { flex: 0 0 auto !important; width: auto !important; }
    div[data-testid="stForm"]:has(.add-tx-card) div[data-testid="stHorizontalBlock"]:has(> div[data-testid="column"]:nth-child(2):last-child) > div:nth-child(2) { flex: 1 1 auto !important; width: auto !important; }
    div[data-testid="stForm"]:has(.add-tx-card) div[role="radiogroup"] { background: rgba(0,0,0,0.3) !important; padding: 6px !important; border-radius: 12px !important; display: flex !important; gap: 8px !important; align-items: center !important; margin: 0 !important; height: 48px !important; border: 1px solid rgba(255,255,255,0.05) !important; min-width: 200px !important; }
    ...
    div[data-testid="stForm"]:has(.add-tx-card) div[role="radiogroup"] label p { font-weight: bold !important; font-size: 1.05rem !important; color: #94a3b8 !important; margin: 0 !important; white-space: nowrap !important; }
    ...
    div[data-testid="stForm"]:has(.add-tx-card) .stButton { display: flex !important; justify-content: flex-end !important; align-items: center !important; margin: 0 !important; width: 100% !important; }
    div[data-testid="stForm"]:has(.add-tx-card) .stButton > button { background: #1e2a44 !important; color: #e0e0e0 !important; padding: 0 24px !important; border-radius: 10px !important; font-size: 1.05rem !important; font-weight: 700 !important; box-shadow: 0 4px 15px rgba(0,0,0,0.25) !important; border: none !important; height: 48px !important; }
    ...
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) div[data-testid="stButton"] button { background: rgba(255,255,255,0.05) !important; border-radius: 8px !important; border: none !important; height: 40px !important; width: 40px !important; display: flex !important; align-items: center !important; justify-content: center !important; margin: 0 auto !important; font-size: 1.2rem !important; }
    div[data-testid="stForm"]:has(.edit-rollout) { background: rgba(0,0,0,0.2) !important; border-left: 3px solid #00ff9d !important; border-radius: 0 0 12px 12px !important; border-top: none !important; padding: 16px !important; margin-top: -24px !important; margin-bottom: 20px !important; position: relative; z-index: 1; box-shadow: inset 0 4px 10px rgba(0,0,0,0.15) !important; }
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.del-warn) { border-color: rgba(255, 77, 77, 0.3) !important; background: rgba(15, 23, 42, 0.95) !important; border-radius: 12px !important; padding: 16px !important; text-align: center !important; margin-bottom: 12px !important; }
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.del-warn) .stButton > button { border-radius: 8px !important; font-weight: 600 !important; width: 100% !important; margin-top: 8px !important; }
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.del-warn) div:nth-child(1) .stButton > button { background: rgba(255, 77, 77, 0.1) !important; color: #ff4d4d !important; border: 1px solid rgba(255, 77, 77, 0.3) !important; }
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.del-warn) div:nth-child(2) .stButton > button { background: rgba(255, 255, 255, 0.05) !important; color: #cbd5e1 !important; border: 1px solid rgba(255, 255, 255, 0.1) !important; }
    @media (max-width: 768px) {
        ...
        div[data-testid="stForm"]:has(.add-tx-card) div[data-testid="stHorizontalBlock"] { display: flex !important; flex-wrap: wrap !important; gap: 12px !important; }
        div[data-testid="stForm"]:has(.add-tx-card) div[data-testid="stHorizontalBlock"]:has(> div[data-testid="column"]:nth-child(4)) > div[data-testid="column"] { min-width: calc(50% - 12px) !important; flex: 1 1 calc(50% - 12px) !important; }
        div[data-testid="stForm"]:has(.add-tx-card) div[data-testid="stHorizontalBlock"]:has(> div[data-testid="column"]:nth-child(2):last-child) > div[data-testid="column"] { min-width: calc(50% - 12px) !important; flex: 1 1 calc(50% - 12px) !important; }
        div[data-testid="stForm"]:has(.add-tx-card) div[role="radiogroup"] { width: 100% !important; }
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] { display: flex !important; flex-wrap: nowrap !important; align-items: center !important; gap: 2px !important; }
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) div[data-testid="column"] { min-width: 0 !important; width: auto !important; flex-shrink: 1 !important; }
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) div[data-testid="column"]:nth-child(1) { flex: 0 0 35px !important; }
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) div[data-testid="column"]:nth-child(2) { flex: 1 1 auto !important; overflow: hidden !important; }
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) div[data-testid="column"]:nth-child(3) { flex: 1.5 1 auto !important; text-align: center; }
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) div[data-testid="column"]:nth-child(4) { flex: 0 0 36px !important; }
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) div[data-testid="column"]:nth-child(5) { flex: 0 0 36px !important; }
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) div[data-testid="stButton"] button { width: 30px !important; height: 30px !important; font-size: 0.9rem !important; }
        .mobile-logo { width: 32px !important; height: 32px !important; } .mobile-tx-ticker { font-size: 0.95rem !important; } .mobile-tx-amount { font-size: 0.95rem !important; } .mobile-tx-sub { font-size: 0.7rem !important; }
    }
    </style>
    """, unsafe_allow_html=True)
    ...
    with st.form("add_crypto", border=False):
        ...
        if submitted and ticker:
            ...
            new_row = pd.DataFrame([{"Datum": date_to_excel_serial(selected_date), "USDC": final_usdc, "Ticker": ticker, "Amount": final_amount, "Price": round(usdc / amount, 8) if amount > 0 else 0.0}])
        ...
        for i, r in df_display.iterrows():
            ...
            if st.session_state.get('confirm_delete_crypto') == orig_idx:
                ...
                with st.container(border=True):
                    ...
                    with col_logo: st.markdown(f"<img src='{get_ticker_logo(r['Ticker'])}' class='mobile-logo' style='width:42px;height:42px;border-radius:50%;object-fit:contain;margin-top:6px;' onerror=\"this.src='https://via.placeholder.com/42/1e2a44/ffffff?text={r['Ticker'][0]}';\">", unsafe_allow_html=True)
                    with col_ticker: st.markdown(f"""<div style="line-height: 1.2; margin-top: 6px; overflow: hidden; text-overflow: ellipsis;"><div class="mobile-tx-ticker" style="font-weight: 700; font-size: 1.15rem; color: #ffffff; white-space: nowrap;">{r['Ticker']}</div><div class="mobile-tx-sub" style="font-size: 0.85rem; color: #94a3b8; white-space: nowrap;">{format_datum(r['Datum'])}</div></div>""", unsafe_allow_html=True)
                    with col_vals: st.markdown(f"""<div style="display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 2px; margin-top: 6px;"><div class="mobile-tx-amount" style="font-weight: 700; font-size: 1.15rem; color: {'#00ff9d' if is_buy else '#ff4d4d'}; white-space: nowrap;">{'+' if is_buy else '-'}{format_holdings(abs(r['Amount']), r['Ticker'])}</div><div class="mobile-tx-sub" style="font-size: 0.85rem; color: #cbd5e1; white-space: nowrap;">{'Spent' if is_buy else 'Received'}: {format_money(abs(r['USDC']))} @ ${format_price(abs(r['USDC'])/abs(r['Amount']) if abs(r['Amount'])>0 else 0)}</div></div>""", unsafe_allow_html=True)
                    with col_edit:
                    ...
                    with st.form(f"edit_crypto_form_{orig_idx}", border=False):
                        ...
                        with e_save: 
                            if st.form_submit_button("💾 Save"):
                                st.session_state.crypto_df.loc[orig_idx] = {"Datum": date_to_excel_serial(new_date), "USDC": new_usdc if tx_type_edit == "Buy" else -new_usdc, "Ticker": new_ticker, "Amount": new_amount if tx_type_edit == "Buy" else -new_amount, "Price": round(new_usdc / new_amount, 8) if new_amount > 0 else 0.0}
                                save_crypto(st.session_state.crypto_df)
...
# ====================== 3. RENDER FIAT PAGE ==============================
# =========================================================================
fiat_container = st.container()
with fiat_container:
    st.markdown('<div id="marker-fiat" class="page-marker"></div>', unsafe_allow_html=True)
    ...
    fees_czk = (pd.to_numeric(st.session_state.fiat_df['Fee'], errors='coerce').fillna(0) * pd.to_numeric(st.session_state.fiat_df['CZK/EUR'], errors='coerce')).sum()
    ...
    with st.container(height=520, border=True):
        ...
        for idx, title in enumerate(["**Date**", "**CZK**", "**EUR**", "**Fee**", "**CZK/EUR**", "**USDC**", "**Del**", "**Edit**"]): h[idx].markdown(title)
        for i, r in df_clean.iterrows():
            cols = st.columns([1.0, 0.9, 0.9, 0.6, 0.9, 1.0, 0.4, 0.4])
            cols[0].write(format_datum(r['Datum']))
            cols[1].write(f"{r['CZK']:,.2f}")
            cols[2].write(f"{r['EUR']:,.2f}")
            cols[3].write(f"{r['Fee']:,.2f}")
            cols[4].write(f"{r['CZK/EUR']:,.5f}")
            cols[5].write(format_money(r['USDC']))
            with cols[6]:
                if st.session_state.get(f'confirm_del_fiat_{i}'):
                    ...
                    if st.button("🗑️", key=f"del_fiat_{i}"):
                        st.session_state[f'confirm_del_fiat_{i}'] = True
                        st.rerun()
            with cols[7]:
                if st.button("✏️", key=f"edit_fiat_{i}"):
    ...
    if 'editing_row' in st.session_state:
        ...
        with st.form("edit_fiat_row"):
            ...
            with col_a: new_datum = date_to_excel_serial(st.date_input("Date", value=datetime(1899, 12, 30) + timedelta(days=int(row['Datum']))))
            ...
            with col_save:
                if st.form_submit_button("💾 Save Changes"):
                    st.session_state.fiat_df.loc[edit_idx] = {"Datum": new_datum, "CZK": new_czk, "EUR": new_eur, "Fee": new_fee, "CZK/EUR": round(new_czk / new_eur, 5) if new_eur > 0 else 0.0, "USDC": new_usdc, "NI": row.get('NI', ""), "GG": row.get('GG', ""), "ER": row.get('ER', "")}
                    save_fiat(st.session_state.fiat_df)
    ...
    with st.form("add_fiat"):
        ...
        with col1: datum = date_to_excel_serial(st.date_input("Date", value=date(2026, 3, 25)))
        ...
        if st.form_submit_button("➕ Add Entry"):
            new_row = pd.DataFrame([{"Datum": datum, "CZK": czk, "EUR": eur, "Fee": fee, "CZK/EUR": round(czk / eur, 5) if eur > 0 else 0.0, "USDC": usdc, "NI": "", "GG": "", "ER": ""}])
...
# --- JS Router & Layout CSS ---
router_css = """
<style>
/* Safe hiding of the Streamlit structural containers holding the marker tags */
div[data-testid="stVerticalBlock"]:has(> div.element-container .page-marker) {
    position: absolute !important;
    visibility: hidden !important;
    opacity: 0 !important;
...
<div id="native-bottom-bar">
    <div class="nav-tab" id="tab-home" onclick="window.switchTab(0)">
        <span class="nav-icon">🏠</span>
        <span class="nav-lbl">Overview</span>
    </div>
    <div class="nav-tab" id="tab-crypto" onclick="window.switchTab(1)">
        <span class="nav-icon">📊</span>
        <span class="nav-lbl">Crypto</span>
    </div>
    <div class="nav-tab" id="tab-fiat" onclick="window.switchTab(2)">
        <span class="nav-icon">💰</span>
...
(function() {
    ...
    // Define global switchTab explicitly
    window.switchTab = function(index) {
        const pages = ['home', 'crypto', 'fiat'];
        localStorage.setItem('activePage', pages[index]);
        
        // Update Bottom Tab Styles
        parentDoc.querySelectorAll('#native-bottom-bar .nav-tab').forEach((t, i) => {
        ...
        // Update Pages Styles
        pages.forEach((p, i) => {
            const marker = parentDoc.getElementById('marker-' + p);
            if(marker) {
                const block = marker.closest('div[data-testid="stVerticalBlock"]');
                if(block) {
                    if(i === index) {
                        block.classList.add('page-active');
                    } else {
                        block.classList.remove('page-active');
                    }
                }
            }
        });

        // Force Highcharts reflow on Home tab
        if(index === 0) {
            setTimeout(() => {
                parentDoc.querySelectorAll('iframe').forEach(ifr => {
                    try { if(ifr.contentWindow && ifr.contentWindow.Highcharts) { ifr.contentWindow.Highcharts.charts.forEach(c => { if(c) c.reflow(); }); } } catch(e){}
                });
            }, 50);
        }
    };

    // Ensure state remains intact on Streamlit rerun
    setTimeout(() => {
        let saved = localStorage.getItem('activePage') || 'home';
        let idx = ['home', 'crypto', 'fiat'].indexOf(saved);
        if(idx === -1) idx = 0;
    ...
    parentDoc.addEventListener('touchend', e => {
        ...
        if (Math.abs(diffX) > Math.abs(diffY) && Math.abs(diffX) > 75) {
            ...
            let curr = ['home', 'crypto', 'fiat'].indexOf(localStorage.getItem('activePage') || 'home');
            if (diffX < 0 && curr < 2) window.switchTab(curr + 1);
            else if (diffX > 0 && curr > 0) window.switchTab(curr - 1);
        }
    }, {passive: true});
})();
</script>
"""
components.html(router_css, height=0, width=0)
