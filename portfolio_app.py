...
.glossy-header:hover::before, .glossy-box:hover::before {
    left: 180%; opacity: 1;
}
.glossy-header { padding: 32px 40px; min-height: 130px; font-size: 29px; font-weight: 700; letter-spacing: 1.8px; display: flex; align-items: center; justify-content: center; gap: 16px; }
.glossy-box { padding: 28px 30px; text-align: center; flex: 1; min-width: 220px; display: flex; flex-direction: column; justify-content: center; }
.glossy-box > div:first-child { font-size: 13.5px; font-weight: 500; letter-spacing: 1.1px; color: #e0e0e0; opacity: 0.9; margin-bottom: 6px; }
.glossy-box > div:last-child { font-size: 27px; font-weight: 700; line-height: 1.05; color: #ffffff; }

/* PERFECTLY TIGHT TABLE - NO GAP, NO HORIZONTAL SCROLLBAR */
.row-inner {
...
.row-inner:hover { transform: translateY(-1px); box-shadow: 0 0 35px var(--glow) !important; }
.scroll-container {
    max-height: 590px;
    overflow-y: auto;
    overflow-x: hidden;
...
def get_all_prices(tickers):
    ...
    coin_ids = {
        ...
        'HBAR': 'hedera-hashgraph', 'TRX': 'tron', 'SUI': 'sui',
        'LINK': 'chainlink', 'BNB': 'binancecoin'
    }
    # CoinGecko (most reliable on Streamlit Cloud)
    try:
        ids = ','.join([coin_ids.get(t, t.lower()) for t in tickers if t != 'USDC'])
        ...
        for t in tickers:
            if t != 'USDC':
                cg_id = coin_ids.get(t, t.lower())
                if cg_id in data:
                    prices[t] = float(data[cg_id]['usd'])
        return prices
    except:
        # Binance fallback
        try:
...
<div style="display:flex;gap:25px;margin-bottom:30px;flex-wrap:wrap;">
    ...
    <div class="glossy-box"><div>PnL %</div><div style="color:{'#00ff9d' if total_pnl_pct>=0 else '#ff4d4d'}">{"▲" if total_pnl_pct>0 else "▼" if total_pnl_pct<0 else ""} {abs(total_pnl_pct):.2f}%</div></div>
        ...
        st.caption(f"Last updated: {datetime.now().strftime('%H:%M:%S')} • Live prices")
