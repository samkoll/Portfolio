...
"""
    ...
    # ====================== CRYPTO TRANSACTIONS ======================
        ...
        # Inject dynamic CSS overrides for layout fixes
        st.markdown("""
        <style>
        ...
        div[data-testid="stForm"] label[data-baseweb="radio"] > div:first-child { display: none !important; }
        ...
        @media (max-width: 768px) {
            /* Force Form inputs to 2-columns on mobile (2x2 grid) */
            div[data-testid="stForm"]:has(.form-compact-marker) div[data-testid="stHorizontalBlock"] {
            ...
            div[data-testid="stForm"]:has(.form-compact-marker) input { font-size: 0.95rem !important; padding: 6px !important; }
            
            /* Force Transaction History rows to stay completely horizontal (inline) on mobile */
            div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row-mobile-marker) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] {
            ...
            .stButton.tx-btn > button { width: 34px !important; height: 34px !important; font-size: 0.95rem !important; }
            .mobile-tx-ticker { font-size: 0.95rem !important; margin-left: 4px !important;}
            .mobile-tx-amount { font-size: 0.95rem !important; white-space: nowrap !important; }
            .mobile-tx-sub { font-size: 0.7rem !important; white-space: nowrap !important; margin-left: 4px !important;}
            .mobile-logo { width: 30px !important; height: 30px !important; }
        ...
        # 1. ADD NEW TRANSACTION CARD (Compact / 2x2 Grid)
        with st.form("add_crypto", border=False):
            st.markdown("<div class='form-compact-marker'></div><h3 style='text-align: center; color: white; margin-top: -5px; margin-bottom: 25px;'>Add New Transaction</h3>", unsafe_allow_html=True)
            ...
            for i, r in df_display.iterrows():
                ...
                if st.session_state.get('confirm_delete_crypto') == orig_idx:
                    with st.container(border=True):
                        st.markdown("<div class='delete-dialog-marker'></div><h4 style='text-align: center; color: #ff4d4d; margin-top: 0; margin-bottom: 10px;'>⚠️ Delete this transaction?</h4>", unsafe_allow_html=True)
                        c_yes, c_no = st.columns(2)
                        with c_yes:
                            if st.button("✅ Yes, Delete", key=f"yes_del_{orig_idx}", use_container_width=True):
                    ...
                    with st.container(border=True):
                        ...
                        # 5 Explicit Columns to perfectly lock items inline
