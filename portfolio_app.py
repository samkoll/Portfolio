...
"""
    ...
    # ====================== CRYPTO TRANSACTIONS ======================
        ...
        /* Instead of hiding the dot, shrink it to 0 so it doesn't break the layout but becomes invisible */
        div[data-testid="stForm"]:has(.add-tx-card) div[role="radiogroup"] label > div:first-child { 
            width: 0px !important; height: 0px !important; opacity: 0 !important; margin: 0 !important; padding: 0 !important;
        } 
        ...
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) div[data-testid="stButton"] button {
            ...
            height: 38px !important; width: 38px !important; display: flex !important; align-items: center !important;
            justify-content: center !important; padding: 0 !important; margin: 0 auto !important; font-size: 1.1rem !important;
        ...
        @keyframes rollDown { from { opacity: 0; max-height: 0px; margin-top: -30px; transform: scaleY(0.9); } to { opacity: 1; max-height: 500px; margin-top: -10px; transform: scaleY(1); } }
        div[data-testid="stForm"]:has(.edit-rollout) {
            animation: rollDown 0.3s ease-out forwards !important; background: #0f172a !important;
            border-left: 3px solid #00ff9d !important; border-radius: 0 0 12px 12px !important;
            border-top: none !important; border-right: none !important; border-bottom: none !important;
            padding: 16px !important; margin-bottom: 20px !important;
        ...
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.del-warn) {
            border: 1px solid #ff4d4d !important; background: rgba(255, 77, 77, 0.05) !important;
            border-radius: 12px !important; padding: 15px !important; text-align: center !important;
        ...
        @media (max-width: 768px) {
            ...
            /* Fix Add Form Mobile 2x2 Grid via explicit CSS calc to avoid auto-stacking */
            div[data-testid="stForm"]:has(.add-tx-card) div[data-testid="stHorizontalBlock"] {
                display: flex !important; flex-direction: row !important; flex-wrap: wrap !important; width: 100% !important; gap: 0 !important;
            }
            div[data-testid="stForm"]:has(.add-tx-card) div[data-testid="stHorizontalBlock"] > div[data-testid="column"] {
                width: calc(50% - 5px) !important; flex: 1 1 calc(50% - 5px) !important; min-width: calc(50% - 5px) !important; margin: 0 2.5px !important;
            }
            ...
            div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] {
                display: flex !important; flex-direction: row !important; flex-wrap: nowrap !important; align-items: center !important; overflow: hidden !important; gap: 4px !important;
            }
            ...
            div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:nth-child(2) { flex: 1 1 auto !important; overflow: hidden !important; } /* Info */
            div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:nth-child(3) { flex: 0 0 34px !important; width: 34px !important; } /* Edit */
            div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:nth-child(4) { flex: 0 0 34px !important; width: 34px !important; } /* Delete */
            
            div[data-testid="stVerticalBlockBorderWrapper"]:has(.tx-row) div[data-testid="stButton"] button {
                width: 30px !important; height: 30px !important; font-size: 0.9rem !important; margin: 0 auto !important;
            }
            
            .mobile-logo { width: 30px !important; height: 30px !important; margin-top: 0 !important; }
        }
        </style>
        """, unsafe_allow_html=True)
        ...
        with st.form("add_crypto", border=False):
            st.markdown("<div class='add-tx-card'></div><h3 style='text-align: center; color: white; margin-top: 0px; margin-bottom: 25px;'>New Transaction</h3>", unsafe_allow_html=True)
            ...
            for i, r in df_display.iterrows():
                ...
                if st.session_state.get('confirm_delete_crypto') == orig_idx:
                    with st.container(border=True):
                        st.markdown("<div class='del-warn'></div><h4 style='color: #ff4d4d; margin-top: 0; margin-bottom: 5px;'>⚠️ Permanently delete?</h4>", unsafe_allow_html=True)
                    ...
                    with st.container(border=True):
                        ...
                        # STRICT 4 COLUMNS: Logo, Details (Left/Right Flexbox), Edit, Delete
                        col_logo, col_info, col_edit, col_del = st.columns([0.6, 5, 0.7, 0.7])
                        
                        with col_logo:
                            st.markdown(f"<img src='{logo_url}' class='mobile-logo' style='width:38px;height:38px;border-radius:50%;object-fit:contain;' onerror=\"this.src='https://via.placeholder.com/38/1e2a44/ffffff?text={r['Ticker'][0]}';\">", unsafe_allow_html=True)
                            
                        with col_info:
                            st.markdown(f"""
                                <div style="display: flex; justify-content: space-between; align-items: center; line-height: 1.2; width: 100%;">
                                    <div style="display: flex; flex-direction: column;">
                                        <span style="font-weight: 700; font-size: 1.05rem; color: #ffffff; white-space: nowrap;">{r['Ticker']}</span>
                                        <span style="font-size: 0.75rem; color: #94a3b8; white-space: nowrap;">{date_str}</span>
                                    </div>
                                    <div style="display: flex; flex-direction: column; text-align: right;">
                                        <span style="font-weight: 700; font-size: 1.05rem; color: {color}; white-space: nowrap;">{sign}{amount_formatted}</span>
                                        <span style="font-size: 0.75rem; color: #cbd5e1; white-space: nowrap;">${price_formatted}</span>
                                    </div>
                                </div>
                            """, unsafe_allow_html=True)
