...
/* MASTER HORIZONTAL CAROUSEL CONTAINER */
...
div[data-testid="stMainBlockContainer"] > div[data-testid="stVerticalBlock"]::-webkit-scrollbar {
    display: none; /* Hide scrollbar Chrome/Safari */
}

/* INDIVIDUAL PAGE CONTAINERS */
div[data-testid="stMainBlockContainer"] > div[data-testid="stVerticalBlock"] > div[data-testid="stVerticalBlock"]:has(.swipe-page) {
    ...
    padding: 40px 14px 120px 14px !important; /* Space for Top/Bottom */
    box-sizing: border-box !important;
    /* Reset flex direction so Streamlit renders normally inside */
    display: flex !important;
...
.glossy-box.swapped > div:first-child { font-size: 12px; font-weight: 600; letter-spacing: 1.5px; text-transform: uppercase; color: #94a3b8; margin-bottom: 6px; line-height: 1.2; }
.glossy-box.swapped > div:last-child { font-size: 27px; font-weight: 700; line-height: 1.05; color: #ffffff; }
...
@media (max-width: 768px) {
    .glossy-header { min-height: 90px; font-size: 22px; padding: 20px 16px; margin-bottom: 24px; }
    ...
    /* Transaction forms mobile wrap override */
...
# ================== GLOBAL CALCULATIONS ==================
# Since all 3 pages are rendered simultaneously, we calculate everything unconditionally
...
(function() {
    ...
    // Attach listeners on interval to ensure DOM is ready
    let attachInt = setInterval(() => {
        const master = doc.querySelector('div[data-testid="stMainBlockContainer"] > div[data-testid="stVerticalBlock"]');
        if (master) {
            clearInterval(attachInt);

            // Restore scroll position
            const saved = win.localStorage.getItem('portfolioScrollPos');
            if (saved) master.scrollLeft = parseInt(saved);

            // Sync Nav Icons on Scroll
            master.addEventListener('scroll', () => {
                clearTimeout(win.scrollTimeout);
                win.scrollTimeout = setTimeout(() => {
                    win.localStorage.setItem('portfolioScrollPos', master.scrollLeft);
...
# ================== PAGE 1: HOME ==================
with st.container():
    st.markdown("<div class='swipe-page' id='page-home'></div>", unsafe_allow_html=True)
    ...
    if history_data_raw:
        ...
        for idx, d in enumerate(history_data_raw):
            ...
            if idx == len(history_data_raw) - 1 and ts == today_ts:
                val, inv = float(total_value), float(fiat_usdc_total)
    ...
    for _, r in df_port.iterrows():
        ticker = r['Ticker']
        if ticker != 'USDC' and pd.notna(r['Value']) and r['Value'] > 0:
            pie_data_js_lines.append(f"{{ name: '{ticker}', y: {r['Value']}, color: '{get_ticker_color(ticker)}' }}")
    ...
    if not pnl_df.empty:
        ...
        def format_pnl_js(series):
            ...
            return ",\n".join([f"{{ name: '{t}', y: {v}, color: '{get_ticker_color(t)}99' }}" for t, v in series.items()])
        ...
        pnl_data_js_dict['1d'] = format_pnl_js(pnl_all - pnl_df_active.iloc[idx_1d])
        pnl_data_js_dict['7d'] = format_pnl_js(pnl_all - pnl_df_active.iloc[idx_7d])
        pnl_data_js_dict['30d'] = format_pnl_js(pnl_all - pnl_df_active.iloc[idx_30d])
        pnl_data_js_dict['1y'] = format_pnl_js(pnl_all - pnl_df_active.iloc[idx_1y])
    ...
    val_data_points, inv_data_points = [], []
    ...
    with c2:
        ...
        st.download_button("💾 Backup", json.dumps(data, indent=2), "portfolio_backup.json", "application/json", use_container_width=True)

# ================== PAGE 2: CRYPTO ==================
with st.container():
    st.markdown("<div class='swipe-page' id='page-crypto'></div>", unsafe_allow_html=True)
...
# ================== PAGE 3: FIAT ==================
with st.container():
    st.markdown("<div class='swipe-page' id='page-fiat'></div>", unsafe_allow_html=True)
