...
/* ---------------------------------------------------
   CRYPTO TAB REVOLUT-STYLE REDESIGN
--------------------------------------------------- */
/* Floating Add Button */
div[data-testid="stVerticalBlock"] div:has(> .btn-add-tx) {
    margin-bottom: 20px;
}
.btn-add-tx button {
    ...
    box-shadow: 0 6px 20px rgba(0, 255, 157, 0.25) !important;
    transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1) !important;
}
.btn-add-tx button:active { transform: scale(0.96) !important; }
...
div[data-testid="stForm"]:has(.add-tx-card) input {
    background: rgba(0,0,0,0.3) !important; border: 1px solid rgba(255,255,255,0.1) !important; color: #fff !important; border-radius: 10px !important;
}

/* Buy/Sell Custom Toggle inside form */
div[data-testid="stForm"]:has(.add-tx-card) div[role="radiogroup"] {
    background: rgba(0,0,0,0.4) !important; padding: 6px !important; border-radius: 12px !important; display: flex !important; gap: 8px !important; height: 50px !important; border: 1px solid rgba(255,255,255,0.05) !important;
}
div[data-testid="stForm"]:has(.add-tx-card) div[role="radiogroup"] label {
    margin: 0 !important; cursor: pointer !important; padding: 0 !important; border-radius: 8px !important; border: 1px solid transparent !important; transition: all 0.3s ease !important; flex: 1 !important; display: flex !important; justify-content: center !important; align-items: center !important; height: 100% !important; background: transparent !important;
}
div[data-testid="stForm"]:has(.add-tx-card) div[role="radiogroup"] label > div:first-child { display: none !important; } 
div[data-testid="stForm"]:has(.add-tx-card) div[role="radiogroup"] label p { font-weight: bold !important; font-size: 1rem !important; color: #94a3b8 !important; margin: 0 !important; line-height: 1 !important; }
...
/* The Modern Transaction Row Container */
div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) {
...
div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker):hover {
    background: rgba(255,255,255,0.06) !important;
}

/* Align buttons properly next to HTML block */
div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] {
    align-items: center !important; 
}
div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) div[data-testid="stButton"] button {
    background: rgba(255,255,255,0.05) !important;
    border-radius: 10px !important; border: none !important;
    height: 42px !important; width: 42px !important; display: flex !important; align-items: center !important; justify-content: center !important; padding: 0 !important; margin: 0 auto !important; font-size: 1.1rem !important; transition: all 0.2s !important; color: #94a3b8 !important;
}
div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) div[data-testid="stButton"] button:hover {
    background: rgba(255,255,255,0.15) !important; color: #fff !important; transform: scale(1.05) !important;
}

/* Edit Rollout */
div[data-testid="stForm"]:has(.edit-rollout) {
    animation: rollDown 0.3s cubic-bezier(0.4, 0, 0.2, 1) forwards !important;
    background: rgba(0,0,0,0.3) !important;
    border-left: 3px solid #00ff9d !important; border-radius: 0 0 16px 16px !important;
    border-top: none !important; border-right: none !important; border-bottom: none !important;
    padding: 16px 20px !important; margin-top: -16px !important; margin-bottom: 20px !important;
...
@media (max-width: 768px) {
    ...
    /* Transaction Row Flex Forcing */
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] { flex-wrap: nowrap !important; gap: 4px !important;}
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:nth-child(1) { flex: 1 1 auto !important; width: auto !important; min-width: 0 !important; }
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:nth-child(2),
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:nth-child(3) { flex: 0 0 38px !important; width: 38px !important; }
    
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) div[data-testid="stButton"] button { height: 36px !important; width: 36px !important; font-size: 0.95rem !important;}
}
</style>
...
def date_to_excel_serial(selected_date: date) -> int:
    return (selected_date - datetime(1899, 12, 30).date()).days
...
def build_portfolio_history(crypto_df, fiat_df, last_prices, hist_dict):
    ...
    else: daily_fiat_usdc = pd.Series(dtype=float)
    ...
    else: daily_crypto_spent = pd.Series(dtype=float)
    ...
    min_date, today = min(all_dates), datetime.now().date()
    ...
    if not crypto.empty:
        ...
        if not crypto_assets.empty:
            holdings = crypto_assets.groupby(['Date', 'Ticker'])['Amount'].sum().unstack(fill_value=0).reindex(date_range, fill_value=0).fillna(0)
    ...
    if not prices_df.empty: prices_df = prices_df.reindex(date_range).ffill().bfill().fillna(0)
    else: prices_df = pd.DataFrame(index=date_range)
    ...
    if not crypto.empty and not crypto_assets.empty:
        cum_invested = crypto_assets.groupby(['Date', 'Ticker'])['USDC'].sum().unstack(fill_value=0).reindex(date_range, fill_value=0).fillna(0).cumsum()
    ...
    history_data = [{'time': int(datetime.combine(d, datetime.min.time()).timestamp()) * 1000, 'value': float(total_portfolio_value.loc[d]), 'invested': float(cum_fiat_usdc.loc[d]), 'btc': float(btc_benchmark_value.loc[d])} for d in date_range]
    ...
    if not common_cols.empty:
        ...
        for coin in sorted(coin_values_last_day.keys(), key=lambda c: coin_values_last_day[c], reverse=True):
            ...
            data_points = [f"[{int(datetime.combine(d, datetime.min.time()).timestamp()) * 1000}, {float(coin_val_series.loc[d])}]" for d in date_range]
...
def calculate_portfolio(crypto_df, fiat_df, live_prices, base_prices):
    ...
    usdc_holdings = fiat_usdc - pd.to_numeric(crypto_df['USDC'], errors='coerce').fillna(0).sum()
    ...
    for ticker in [t for t in crypto_df['Ticker'].unique() if t != 'USDC']:
        ...
        portfolio.append({'Ticker':ticker,'Holdings':total_holdings,'USDC':total_invested,'AVG':total_invested/total_holdings if total_holdings > 0 else 0,'Live':live_price,'PnL':pnl,'PnL %':(pnl/total_invested*100) if total_invested > 0 else 0,'Value':value, 'Price7d':bp['7d'], 'Price30d':bp['30d'], 'Price90d':bp['90d'], 'PriceYTD':bp['ytd']})
...
def format_money(val):
    try: return f"${float(val):,.2f}" if pd.notna(val) and float(val) >= 0 else f"-${-float(val):,.2f}" if pd.notna(val) else ""
    except: return ""

def format_holdings(val, ticker=None):
    try: return f"{float(val):,.6f}".replace(',', '.') if ticker == "BTC" else f"{float(val):,.4f}".replace(',', '.')
    except: return str(val)

def format_percent(val): return f"{float(val):.2f}%" if pd.notna(val) else ""
def format_price(val): return f"{float(val):.4f}" if pd.notna(val) and abs(float(val)) < 1 else f"{float(val):,.2f}" if pd.notna(val) else ""
...
# TRUE SWIPE ANIMATION HANDLER & ROUTER
components.html("""
    <script>
    (function() {
        ...
        parentDoc.addEventListener('touchend', e => {
            ...
            if (xDiff > yDiff && xDiff > 80) {
                ...
                let currentTab = 0;
                const tabs = parentDoc.querySelectorAll('button[data-baseweb="tab"]');
                tabs.forEach((t, i) => { if(t.getAttribute('aria-selected') === 'true') currentTab = i; });

                const mainContainer = parentDoc.querySelector('div[data-testid="stMainBlockContainer"]');
                if (!mainContainer) return;

                if (endX < startX && currentTab < 2) { // Swipe Left (Next tab)
                    executeSwipe(currentTab + 1, -1, mainContainer);
                } else if (endX > startX && currentTab > 0) { // Swipe Right (Prev tab)
                    executeSwipe(currentTab - 1, 1, mainContainer);
                }
            }
        }, {passive: true});

        function executeSwipe(newIndex, direction, container) {
            const offset = direction * -30; // Slide slightly in direction of swipe
            
            // Phase 1: Slide out
            container.style.transition = 'transform 0.15s ease-in, opacity 0.15s ease-in';
            ...
            setTimeout(() => {
                ...
                // Phase 2: Instant jump to opposite side for smooth entry
...
with tab_home:
    ...
    if st.session_state.portfolio_cache.get('hash') != current_hash:
        ...
        st.session_state.portfolio_cache = {
            'hash': current_hash, 'df_port': df_port, 'total_value': total_value,
            'total_pnl': total_pnl, 'total_pnl_pct': total_pnl_pct,
            'history_data_raw': history_data_raw, 'allocation_series_js': allocation_series_js,
...
"""
    ...
    for _, r in df_port.iterrows():
        if r['Ticker'] != 'USDC' and pd.notna(r['Value']) and r['Value'] > 0:
            pie_data_js_lines.append(f"{{ name: '{r['Ticker']}', y: {r['Value']}, color: '{get_ticker_color(r['Ticker'])}' }}")
    pie_data_js = ",\n".join(pie_data_js_lines)

    pnl_data_js_dict = {'all': '', '1y': '', '30d': '', '7d': '', '1d': ''}
    if not pnl_df.empty:
        valid_cols = [c for c in [t for t in df_port['Ticker'] if t != 'USDC'] if c in pnl_df.columns]
        pnl_df_active = pnl_df[valid_cols] if valid_cols else pnl_df
        def format_pnl_js(series): return ",\n".join([f"{{ name: '{ticker}', y: {val}, color: '{get_ticker_color(ticker)}99' }}" for ticker, val in series.dropna().sort_values(ascending=True).items()])
        p_all = pnl_df_active.iloc[-1]
        pnl_data_js_dict['all'] = format_pnl_js(p_all)
        pnl_data_js_dict['1d'] = format_pnl_js(p_all - pnl_df_active.iloc[-2 if len(pnl_df_active) >= 2 else 0])
        pnl_data_js_dict['7d'] = format_pnl_js(p_all - pnl_df_active.iloc[-8 if len(pnl_df_active) >= 8 else 0])
        pnl_data_js_dict['30d'] = format_pnl_js(p_all - pnl_df_active.iloc[-31 if len(pnl_df_active) >= 31 else 0])
        pnl_data_js_dict['1y'] = format_pnl_js(p_all - pnl_df_active.iloc[-365 if len(pnl_df_active) >= 365 else 0])
    ...
    roi_data_js = ",\n".join([f"{{ name: '{r['Ticker']}', y: {r['PnL %']}, color: '{get_ticker_color(r['Ticker'])}99' }}" for _, r in df_roi.iterrows() if pd.notna(r['PnL %'])])
    ...
    <head>
        ...
        <style>
            ...
            .charts-scroll-wrapper {{ width: 100%; overflow-y: hidden; overflow-x: auto; padding: 6px 0px 6px 0px; scroll-snap-type: x mandatory; -webkit-overflow-scrolling: touch; scrollbar-width: none; -ms-overflow-style: none; }}
            ...
            .chart-placeholder[data-type="history"], .chart-placeholder[data-type="allocation"] {{ width: 600px; flex: 0 0 600px; height: 340px; }}
            .chart-placeholder[data-type="pnl"], .chart-placeholder[data-type="roi"], .chart-placeholder[data-type="daily"] {{ width: 400px; flex: 0 0 400px; height: 340px; }}
            .chart-placeholder[data-type="inv-val"] {{ width: 500px; flex: 0 0 500px; height: 340px; }}
            .chart-box {{ width: 100%; height: 100%; background: rgba(15, 23, 42, 0.4); border: 1px solid rgba(255,255,255,0.05); border-radius: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); touch-action: pan-x pan-y; position: relative; display: flex; flex-direction: column; }}
                ...
                <div class="chart-placeholder" data-type="history">
                    <div id="history-wrapper" class="chart-box">
                        <div class="chart-header">
                            ...
                            <div class="chart-controls hist-controls"><button class="active" data-range="all">All</button><button data-range="1w">1W</button><button data-range="1m">1M</button><button data-range="1y">1Y</button><button data-range="ytd">YTD</button></div>
                        </div>
                ...
                <div class="chart-placeholder" data-type="pnl">
                    <div id="pnl-wrapper" class="chart-box">
                        <div class="chart-header">
                            ...
                            <div class="chart-controls pnl-controls"><button class="active" data-range="all">All</button><button data-range="1d">Today</button><button data-range="7d">1W</button><button data-range="30d">1M</button><button data-range="1y">1Y</button></div>
                        </div>
        ...
        <script>
            ...
            function toggleExpandChart(wrapperId) {{
                ...
                if (el.classList.contains('expanded-chart')) {{
                    ...
                    const finishClose = (e) => {{ if (e && e.propertyName !== 'transform') return; el.removeEventListener('transitionend', finishClose); clearTimeout(el._closeTimeout); if (parentIframe) parentIframe.classList.remove('fullscreen-mode'); el.style.cssText = ''; wrapper.style.overflowX = 'auto'; document.querySelectorAll('.chart-box').forEach(c => {{ c.style.opacity = '1'; c.style.pointerEvents = 'auto'; }}); }};
                    el.addEventListener('transitionend', finishClose); el._closeTimeout = setTimeout(finishClose, 450);
                    return;
                }}
                
                document.querySelectorAll('.chart-box').forEach(c => {{ if (c.id !== wrapperId) {{ c.style.transition = 'opacity 0.15s ease'; c.style.opacity = '0'; c.style.pointerEvents = 'none'; }} }});
                const chartRect = el.getBoundingClientRect(); let visualTop = chartRect.top, visualLeft = chartRect.left;
                if (parentIframe) {{ const iframeRect = parentIframe.getBoundingClientRect(); visualTop += iframeRect.top; visualLeft += iframeRect.left; }}
                if (parentIframe) parentIframe.classList.add('fullscreen-mode');
                overlay.classList.add('active'); wrapper.style.overflowX = 'visible';
                const origW = chartRect.width, origH = chartRect.height, targetScale = Math.min((screenW * 0.95) / origW, (screenH * 0.70) / origH);
                el.setAttribute('data-orig-top', visualTop); el.setAttribute('data-orig-left', visualLeft);
                el.style.position = 'fixed'; el.style.top = '0px'; el.style.left = '0px'; el.style.width = origW + 'px'; el.style.height = origH + 'px'; el.style.margin = '0'; el.style.zIndex = '1001'; el.style.transformOrigin = 'top left'; el.style.transition = 'none'; el.style.transform = `translate(${{visualLeft}}px, ${{visualTop}}px) scale(1)`;
                void el.offsetWidth;
                el.classList.add('expanded-chart'); el.style.transition = 'transform 0.4s cubic-bezier(0.16, 1, 0.3, 1), background-color 0.4s ease, box-shadow 0.4s ease'; el.style.transform = `translate(${{(screenW - origW * targetScale) / 2}}px, {{(screenH - origH * targetScale) / 2}}px) scale(${{targetScale}})`;
            }}

            function setupDoubleTap(elementId) {{ const el = document.getElementById(elementId); if (!el) return; let lastTap = 0; el.addEventListener('touchend', function(e) {{ const tapLength = new Date().getTime() - lastTap; if (tapLength < 400 && tapLength > 0) {{ toggleExpandChart(elementId); e.preventDefault(); e.stopPropagation(); }} lastTap = new Date().getTime(); }}); }}
            ['pie-container', 'history-wrapper', 'pnl-wrapper', 'roi-wrapper', 'daily-wrapper', 'allocation-container', 'inv-val-container'].forEach(setupDoubleTap);
            document.getElementById('chart-overlay').addEventListener('click', () => {{ document.querySelectorAll('.expanded-chart').forEach(el => toggleExpandChart(el.id)); }});

            const chartScroll = document.getElementById('chartsScrollContainer');
            if (chartScroll) {{ chartScroll.addEventListener('wheel', (evt) => {{ if (Math.abs(evt.deltaY) > Math.abs(evt.deltaX)) {{ evt.preventDefault(); chartScroll.scrollBy({{ left: evt.deltaY > 0 ? 200 : -200, behavior: 'smooth' }}); }} }}, {{ passive: false }}); }}
        </script>
    </body>
    </html>
    """
...
<div class="usdc-banner-left"><img src="{get_ticker_logo('USDC')}" onerror="this.src='https://via.placeholder.com/42/1e2a44/ffffff?text=U';"><div class="usdc-banner-title">USDC <span class="usdc-banner-subtitle">(Available Cash)</span></div></div>
...
"""
    ...
    for _, r in df_port.iterrows():
        ...
        cards_html += f"""
<div class="flip-card" data-ticker="{r['Ticker']}" data-holdings="{r['Holdings']}" data-invested="{r['USDC']}" data-current-price="{r['Live']}" data-avg-price="{r['AVG']}" data-price-7d="{r.get('Price7d', r['Live'])}" data-price-30d="{r.get('Price30d', r['Live'])}" data-price-90d="{r.get('Price90d', r['Live'])}" data-price-ytd="{r.get('PriceYTD', r['Live'])}" data-border="{border_color}" data-chart-color="{border_color}">
<div class="flip-card-inner">
    <div class="flip-card-front">
        <div class="card-header" style="align-items: center; margin-bottom: 4px;">
            <div class="header-left-container"><div class="header-logo-row"><img src="{get_ticker_logo(r['Ticker'])}" style="height:44px;width:44px;border-radius:50%;object-fit:contain;" onerror="this.src='https://via.placeholder.com/44/1e2a44/ffffff?text={r['Ticker'][0]}';"><span style="font-weight:700;font-size:1.3rem;margin-left:12px;color:#ffffff;">{r['Ticker']}</span></div></div>
            <div class="header-right"><div class="stat-group" style="align-items: flex-end;"><div class="current-value" style="font-size: 1.4rem;">${format_price(r['Live'])}</div><div class="stat-label" style="font-size: 0.75rem; margin-top: 2px; color: #94a3b8;">Avg: <span class="privacy-val" style="color: #cbd5e1; font-weight: 600;">${format_price(r['AVG'])}</span></div></div></div>
        </div>
        <div class="metrics-row">
            <div class="metric-col"><span class="metric-lbl">24H</span><span class="metric-val" id="change-24h-{r['Ticker']}">...</span></div>
            <div class="metric-col"><span class="metric-lbl">7D</span><span class="metric-val" id="change-7d-{r['Ticker']}">...</span></div>
            <div class="metric-col"><span class="metric-lbl">30D</span><span class="metric-val" id="change-30d-{r['Ticker']}">...</span></div>
            <div class="metric-col"><span class="metric-lbl">90D</span><span class="metric-val" id="change-90d-{r['Ticker']}">...</span></div>
            <div class="metric-col"><span class="metric-lbl">YTD</span><span class="metric-val" id="change-ytd-{r['Ticker']}">...</span></div>
        </div>
        <div class="card-content">
            <div class="label-value-row" style="margin-top:4px;"><span class="label">Holdings</span><span class="value privacy-val">{format_holdings(r['Holdings'], r['Ticker'])}</span></div>
            ...
            <div class="label-value-row"><span class="label">PnL %</span><span class="value card-pnl-pct privacy-val" style="color:{pnl_color};">{arrow} {format_percent(abs(r['PnL %'])) if pd.notna(r['PnL %']) else ""}</span></div>
    ...
    <div class="flip-card-back">
        <a href="https://www.tradingview.com/chart/?symbol=BINANCE:{r['Ticker']}USDT" target="_blank" class="tv-external-btn" title="Open in TradingView App/Web to save drawings">{TV_ICON}<div style="margin-left: 6px; display:flex;">{EXTERNAL_LINK_ICON}</div></a>
        <div class="chart-container"><canvas id="chart-{r['Ticker']}"></canvas><div class="chart-loading" id="loading-{r['Ticker']}">Loading chart...</div></div>
    </div>
</div>
</div>
"""
...
<style>
    * {{ box-sizing: border-box; }}
    ...
    .scroll-wrapper {{ width: 100%; overflow-y: hidden; overflow-x: auto; padding: 20px 0px; scroll-snap-type: x mandatory; -webkit-overflow-scrolling: touch; scrollbar-width: none; -ms-overflow-style: none; }}
    ...
    .metrics-row {{ display: flex; justify-content: space-between; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 8px 12px; margin: 14px 0; }}
    ...
    @media (max-width: 700px) {{
        .flip-card {{ flex: 0 0 85vw; height: 305px; }} .coin-grid {{ padding: 0 7.5vw; gap: 16px; }}
        ...
        .metrics-row {{ padding: 6px 8px; }} .metric-val {{ font-size: 0.75rem; }} .metric-lbl {{ font-size: 0.55rem; }}
    }}
</style>
</head>
<body>
...
<script>
(function() {{
    function closeAllOpenUI(e) {{ if (!(e && e.target && typeof e.target.closest === 'function' && e.target.closest('.flip-card'))) {{ document.querySelectorAll('.flip-card.flipped').forEach(c => {{ c.classList.remove('flipped'); c.classList.remove('touch-hover'); }}); }} }}
    ['click', 'touchstart'].forEach(evt => document.addEventListener(evt, closeAllOpenUI, {{ passive: true }}));

    let lastDashState = null;
    setInterval(() => {{
        try {{
            const dt = window.parent.document.getElementById('dash-toggle'), dtUsdc = window.parent.document.getElementById('dash-toggle-usdc');
            if (dt) {{
                if (dtUsdc && dtUsdc.checked !== dt.checked) dtUsdc.checked = dt.checked;
                if (dt.checked !== lastDashState) {{
                    lastDashState = dt.checked;
                    document.body.classList.toggle('privacy-mode', !dt.checked);
                    localStorage.setItem('dashboardOpen', dt.checked);
                }}
            }}
        }} catch(e) {{}}
    }}, 150);
    ...
    try {{
        const dt = window.parent.document.getElementById('dash-toggle'), dtUsdc = window.parent.document.getElementById('dash-toggle-usdc');
        if (dt) {{
            const saved = localStorage.getItem('dashboardOpen') === 'true';
            dt.checked = saved; if(dtUsdc) dtUsdc.checked = saved;
            document.body.classList.toggle('privacy-mode', !saved);
            lastDashState = saved;
        }}
        ['click', 'touchstart'].forEach(evt => window.parent.document.addEventListener(evt, closeAllOpenUI, {{ passive: true }}));
    }} catch(e) {{}}
    ...
    function updateMetricUI(card, ticker, period, basePrice, currentPrice) {{
        ...
        const change = ((currentPrice - basePrice) / basePrice) * 100, sign = change >= 0 ? '▲' : '▼', color = change >= 0 ? '#00ff9d' : '#ff4d4d';
    ...
    async function updateLivePrices() {{
        ...
        const mappedTickers = cards.map(c => symbolMap[c.getAttribute('data-ticker').toUpperCase()] || c.getAttribute('data-ticker').toUpperCase());

        try {{
            const resp = await fetch(`https://min-api.cryptocompare.com/data/pricemultifull?fsyms=${{mappedTickers.join(',')}}&tsyms=USD`);
            const data = await resp.json();
            
            let totalCoinValue = 0, totalCoinInvested = 0, tickerValues = {{}}, tickerDiffs = {{}}, tickerRoi = {{}}, ticker24h = {{}};
            
            cards.forEach(card => {{
                const ticker = card.getAttribute('data-ticker'), sym = symbolMap[ticker.toUpperCase()] || ticker.toUpperCase();
                const holdings = parseFloat(card.getAttribute('data-holdings')), invested = parseFloat(card.getAttribute('data-invested'));
                ...
                if (data.RAW && data.RAW[sym] && data.RAW[sym].USD) {{
                    if (data.RAW[sym].USD.PRICE !== undefined) {{
                        const newPrice = data.RAW[sym].USD.PRICE, oldVal = holdings * price;
                        price = newPrice; card.setAttribute('data-current-price', price);
                        const value = holdings * price, pnl = value - invested, pnlPct = invested > 0 ? (pnl / invested) * 100 : 0;
                        tickerValues[ticker] = value; tickerDiffs[ticker] = value - oldVal; tickerRoi[ticker] = pnlPct;
                        
                        const curEl = card.querySelector('.current-value'); if(curEl) curEl.innerText = '$' + (price < 1 ? price.toFixed(4) : price.toLocaleString('en-US', {{minimumFractionDigits: 2, maximumFractionDigits: 2}}));
                        const valEl = card.querySelector('.total-value'); if(valEl) valEl.innerText = '$' + value.toLocaleString('en-US', {{minimumFractionDigits: 2, maximumFractionDigits: 2}});
                        const pnlEl = card.querySelector('.card-pnl'); if(pnlEl) {{ pnlEl.innerText = (pnl >= 0 ? '▲ $' : '▼ $') + Math.abs(pnl).toLocaleString('en-US', {{minimumFractionDigits: 2, maximumFractionDigits: 2}}); pnlEl.style.color = pnl >= 0 ? '#00ff9d' : '#ff4d4d'; }}
                        const pnlPctEl = card.querySelector('.card-pnl-pct'); if(pnlPctEl) {{ pnlPctEl.innerText = (pnl >= 0 ? '▲ ' : '▼ ') + Math.abs(pnlPct).toFixed(2) + '%'; pnlPctEl.style.color = pnl >= 0 ? '#00ff9d' : '#ff4d4d'; }}
                        
                        updateMetricUI(card, ticker, '7d', parseFloat(card.getAttribute('data-price-7d')), price);
                        updateMetricUI(card, ticker, '30d', parseFloat(card.getAttribute('data-price-30d')), price);
                        updateMetricUI(card, ticker, '90d', parseFloat(card.getAttribute('data-price-90d')), price);
                        updateMetricUI(card, ticker, 'ytd', parseFloat(card.getAttribute('data-price-ytd')), price);
                        
                        if (window.chartCache && window.chartCache[ticker] && window.chartCache[ticker].chartObj) {{
                            const chart = window.chartCache[ticker].chartObj, dLen = chart.data.datasets[0].data.length;
                            if (dLen > 0) {{ chart.data.datasets[0].data[dLen - 1] = price; chart.update('none'); }}
                        }}
                    }}
                    if (changeSpan && data.RAW[sym].USD.CHANGEPCT24HOUR !== undefined) {{
                        const change = data.RAW[sym].USD.CHANGEPCT24HOUR; ticker24h[ticker] = change;
                        changeSpan.innerHTML = `<span style="color:${{change >= 0 ? '#00ff9d' : '#ff4d4d'}};">${{change >= 0 ? '▲' : '▼'}} ${{Math.abs(change).toFixed(2)}}%</span>`;
                    }}
                }} else if (changeSpan && changeSpan.innerText === '...') changeSpan.innerHTML = `N/A`;
                
                totalCoinValue += (holdings * price); totalCoinInvested += invested;
            }});
            
            const usdcHoldings = {usdc_holdings};
            const totalPortfolioValue = totalCoinValue + usdcHoldings, totalPnL = totalCoinValue - totalCoinInvested;
            const totalInvestedBase = totalPortfolioValue - totalPnL, totalPnLPct = totalInvestedBase !== 0 ? (totalPnL / totalInvestedBase) * 100 : 0;
            const dColor = totalPnL >= 0 ? '#00ff9d' : '#ff4d4d';
            
            const pd = window.parent.document, dV = pd.getElementById('dash-total-value'), dP = pd.getElementById('dash-pnl'), dPP = pd.getElementById('dash-pnl-pct');
            if (dV) dV.innerText = '$' + totalPortfolioValue.toLocaleString('en-US', {{minimumFractionDigits: 2, maximumFractionDigits: 2}});
            if (dP) {{ dP.innerText = (totalPnL >= 0 ? '▲ $' : '▼ $') + Math.abs(totalPnL).toLocaleString('en-US', {{minimumFractionDigits: 2, maximumFractionDigits: 2}}); dP.style.color = dColor; }}
            if (dPP) {{ dPP.innerText = (totalPnL >= 0 ? '▲ ' : '▼ ') + Math.abs(totalPnLPct).toFixed(2) + '%'; dPP.style.color = dColor; }}

            try {{
                let hcWin = null; const iframes = pd.querySelectorAll('iframe');
                ...
                if (hcWin) {{
                    ...
                    const pc = HC.charts.find(c => c && c.renderTo.id === 'pie-container'), hc = HC.charts.find(c => c && c.renderTo.id === 'history-container'), pnlc = HC.charts.find(c => c && c.renderTo.id === 'pnl-container'), rc = HC.charts.find(c => c && c.renderTo.id === 'roi-container'), dc = HC.charts.find(c => c && c.renderTo.id === 'daily-container'), ac = HC.charts.find(c => c && c.renderTo.id === 'allocation-container'), ivc = HC.charts.find(c => c && c.renderTo.id === 'inv-val-container');
                    
                    if (pc && pc.series[0]) {{ pc.series[0].points.forEach(p => {{ if (tickerValues[p.name] !== undefined) p.update({{y: tickerValues[p.name]}}, false); }}); pc.redraw(true); }}
                    if (hc && hc.series[0] && hc.series[0].points.length) {{ hc.series[0].points[hc.series[0].points.length - 1].update({{y: totalPortfolioValue}}, false); hc.redraw(true); }}
                    if (pnlc && pnlc.series[0]) {{
                        pnlc.series[0].points.forEach(p => {{ if (tickerDiffs[p.name] !== undefined) p.update({{y: p.y + tickerDiffs[p.name]}}, false); }});
                        if (hcWin.pnlDataMap) Object.values(hcWin.pnlDataMap).forEach(arr => arr.forEach(pt => {{ if (tickerDiffs[pt.name] !== undefined) pt.y += tickerDiffs[pt.name]; }}));
                        pnlc.redraw(true);
                    }}
                    if (rc && rc.series[0]) {{ rc.series[0].points.forEach(p => {{ if (tickerRoi[p.name] !== undefined) p.update({{y: tickerRoi[p.name]}}, false); }}); rc.redraw(true); }}
                    if (dc && dc.series[0]) {{ dc.series[0].points.forEach(p => {{ if (ticker24h[p.name] !== undefined) {{ const v = ticker24h[p.name]; p.update({{y: v, color: v >= 0 ? 'rgba(0, 255, 157, 0.65)' : 'rgba(255, 77, 77, 0.65)'}}, false); }} }}); dc.redraw(true); }}
                    if (ac) {{ ac.series.forEach(s => {{ if (tickerValues[s.name] !== undefined && s.points.length) s.points[s.points.length - 1].update({{y: tickerValues[s.name]}}, false); }}); ac.redraw(true); }}
                    if (ivc && ivc.series[1]) {{ ivc.series[1].points.forEach(p => {{ if (tickerValues[p.name] !== undefined) p.update({{y: tickerValues[p.name]}}, false); }}); ivc.redraw(true); }}
                }}
            }} catch (e) {{}}
        }} catch (e) {{}}
    }}
    ...
    function restoreFlippedState() {{
        const saved = localStorage.getItem('flippedCards'); if (!saved) return;
        const flippedTickers = JSON.parse(saved);
        document.querySelectorAll('.flip-card').forEach(card => {{
            const ticker = card.getAttribute('data-ticker');
            if (flippedTickers.includes(ticker)) {{
                card.classList.add('flipped'); card.classList.add('touch-hover');
                if (!chartCache[ticker] || !chartCache[ticker].chartObj) renderChart(card, ticker, parseFloat(card.getAttribute('data-current-price')), parseFloat(card.getAttribute('data-avg-price')), card.getAttribute('data-chart-color'));
            }}
        }});
        localStorage.removeItem('flippedCards');
    }}

    window.chartCache = window.chartCache || {{}}; const chartCache = window.chartCache;
    
    async function fetchHistoricalData(ticker) {{
        const sym = {{'BTC':'BTC','ETH':'ETH','SOL':'SOL','HBAR':'HBAR','XRP':'XRP','BNB':'BNB','TRX':'TRX','LINK':'LINK','SUI':'SUI'}}[ticker.toUpperCase()];
        if (!sym) return null;
        try {{
            const resp = await fetch(`https://min-api.cryptocompare.com/data/v2/histoday?fsym=${{sym}}&tsym=USD&limit=30`);
            const data = await resp.json();
            if (data?.Data?.Data) return {{ labels: data.Data.Data.map(d => {{ const dt = new Date(d.time * 1000); return `${{dt.getDate().toString().padStart(2,'0')}}/${{(dt.getMonth()+1).toString().padStart(2,'0')}}`; }}), prices: data.Data.Data.map(d => d.close) }};
        }} catch(e) {{}} return null;
    }}
    
    async function renderChart(card, ticker, currentPrice, avgPrice, chartColor) {{
        const canvas = card.querySelector(`canvas#chart-${{ticker}}`), loadDiv = card.querySelector(`.chart-loading`);
        if (!canvas || (chartCache[ticker] && chartCache[ticker].chart)) {{ if(loadDiv) loadDiv.style.display = 'none'; return; }}
        if (loadDiv) loadDiv.style.display = 'block';
        const hist = await fetchHistoricalData(ticker);
        if (!hist || !hist.prices.length) {{ if (loadDiv) loadDiv.innerText = 'Failed to load chart data'; return; }}
        ...
        chartCache[ticker] = {{ chartObj: new Chart(canvas.getContext('2d'), {{ type: 'line', data: {{ labels: hist.labels, datasets: [{{ label: 'Close Price ($)', data: hist.prices, borderColor: chartColor, backgroundColor: chartColor + '20', borderWidth: 2, fill: true, tension: 0.2, pointRadius: 2, pointBackgroundColor: chartColor }}, {{ label: 'Avg Price ($)', data: new Array(hist.labels.length).fill(avgPrice), borderColor: '#ffaa00', borderWidth: 2, borderDash: [5, 5], fill: false, pointRadius: 0, type: 'line' }}] }}, options: {{ responsive: true, maintainAspectRatio: false, plugins: {{ legend: {{ display: false }}, tooltip: {{ mode: 'index', intersect: false }} }}, scales: {{ x: {{ ticks: {{ color: '#ccc', maxRotation: 45, autoSkip: true, maxTicksLimit: 6 }}, grid: {{ color: 'rgba(255,255,255,0.1)' }} }}, y: {{ position: 'right', ticks: {{ color: '#ccc' }}, grid: {{ color: 'rgba(255,255,255,0.1)' }} }} }} }} }}), data: hist }};
        if (loadDiv) loadDiv.style.display = 'none';
    }}
    
    document.querySelectorAll('.flip-card').forEach(card => {{
        const ticker = card.getAttribute('data-ticker'), cPrice = parseFloat(card.getAttribute('data-current-price'));
        card.style.setProperty('--border', card.getAttribute('data-border'));
        updateMetricUI(card, ticker, '7d', parseFloat(card.getAttribute('data-price-7d')), cPrice); updateMetricUI(card, ticker, '30d', parseFloat(card.getAttribute('data-price-30d')), cPrice); updateMetricUI(card, ticker, '90d', parseFloat(card.getAttribute('data-price-90d')), cPrice); updateMetricUI(card, ticker, 'ytd', parseFloat(card.getAttribute('data-price-ytd')), cPrice);
        card.querySelector('.flip-card-front').addEventListener('click', e => {{ e.stopPropagation(); if (!card.classList.contains('flipped')) {{ card.classList.add('flipped', 'touch-hover'); if (!chartCache[ticker] || !chartCache[ticker].chartObj) renderChart(card, ticker, cPrice, parseFloat(card.getAttribute('data-avg-price')), card.getAttribute('data-chart-color')); }} }});
        card.querySelector('.flip-card-back').addEventListener('click', () => card.classList.remove('flipped', 'touch-hover'));
        const extBtn = card.querySelector('.tv-external-btn'); if (extBtn) extBtn.addEventListener('click', e => e.stopPropagation());
    }});
    
    updateLivePrices(); restoreFlippedState();
    const rKey = '{st.session_state.refresh_key}';
    if (window.oldRefreshKey && window.oldRefreshKey !== rKey) {{ for (let k in chartCache) if (chartCache[k].chartObj) chartCache[k].chartObj.destroy(); window.chartCache = {{}}; }}
    window.oldRefreshKey = rKey;
}})();
</script>
</body>
</html>
...
with tab_crypto:
    ...
    st.markdown("""
    <style>
    /* Revolut-style Layout Overrides for Crypto Tab */
    
    /* 1. Floating Action Button for Add Transaction */
    div[data-testid="stVerticalBlock"] div:has(> .btn-add-tx) {
        margin-bottom: 20px;
    }
    .btn-add-tx button {
    ...
    /* 2. Glass-Morphic Add/Edit Form Card */
    div[data-testid="stForm"]:has(.add-tx-card) {
        background: rgba(15, 23, 42, 0.7) !important;
        backdrop-filter: blur(12px) !important;
        ...
        box-shadow: 0 10px 40px rgba(0,0,0,0.5) !important;
        margin-bottom: 24px !important;
        animation: rollDown 0.35s cubic-bezier(0.16, 1, 0.3, 1) forwards;
    }
    @keyframes rollDown {
    ...
    /* Modern Row Container wrapping HTML and Streamlit Buttons perfectly */
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) {
        background: rgba(20, 28, 43, 0.6) !important;
        border: 1px solid rgba(255,255,255,0.04) !important;
        border-radius: 16px !important;
    ...
    /* Align Streamlit buttons vertically center with HTML content */
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] {
        align-items: center !important; 
    }
    ...
    /* Pure HTML Row Internals */
    .modern-tx-row {
        display: flex; justify-content: space-between; align-items: center; width: 100%;
    }
    .tx-left { display: flex; align-items: center; gap: 12px; }
    .tx-logo { width: 38px; height: 38px; border-radius: 50%; object-fit: contain; }
    .tx-title { font-weight: 700; font-size: 1.1rem; color: #fff; line-height: 1.2; }
    .tx-sub { font-size: 0.75rem; color: #94a3b8; margin-top: 2px; }
    .tx-right { text-align: right; margin-right: 8px; }
    .tx-amount { font-weight: 700; font-size: 1.05rem; }
    .tx-buy { color: #00ff9d; }
    .tx-sell { color: #ff4d4d; }
    .tx-value { font-size: 0.75rem; color: #cbd5e1; margin-top: 2px;}

    /* Edit Docking (slides out from under the row) */
    div[data-testid="stForm"]:has(.edit-rollout) {
        animation: rollDown 0.3s cubic-bezier(0.4, 0, 0.2, 1) forwards !important;
        background: rgba(0,0,0,0.3) !important;
        border-left: 3px solid #00ff9d !important; border-radius: 0 0 16px 16px !important;
    ...
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.del-warn) {
        border-color: rgba(255, 77, 77, 0.3) !important; background: rgba(15, 23, 42, 0.95) !important; border-radius: 12px !important; padding: 16px !important; text-align: center !important; margin-bottom: 12px !important; box-shadow: 0 4px 20px rgba(0,0,0,0.4) !important;
    }
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.del-warn) .stButton > button {
        border-radius: 8px !important; font-weight: 600 !important; width: 100% !important; margin-top: 8px !important;
    }
    ...
    @media (max-width: 768px) {
        /* Add Form Wrap */
        div[data-testid="stForm"]:has(.add-tx-card) div[data-testid="stHorizontalBlock"] { display: flex !important; flex-direction: row !important; flex-wrap: wrap !important; gap: 12px !important; }
        div[data-testid="stForm"]:has(.add-tx-card) div[data-testid="stHorizontalBlock"] > div[data-testid="column"] { min-width: calc(50% - 12px) !important; width: calc(50% - 12px) !important; flex: 1 1 calc(50% - 12px) !important; }
        div[data-testid="stForm"]:has(.add-tx-card) div[role="radiogroup"] { width: 100% !important; min-width: 0 !important; }
        
        /* Modern Row Compression */
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) { padding: 8px 10px !important; }
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] { flex-wrap: nowrap !important; gap: 4px !important;}
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:nth-child(1) { flex: 1 1 auto !important; width: auto !important; min-width: 0 !important; }
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:nth-child(2),
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) > div > div[data-testid="stVerticalBlock"] > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:nth-child(3) { flex: 0 0 36px !important; width: 36px !important; }
        
        .tx-logo { width: 32px; height: 32px; }
        .tx-title { font-size: 1rem; }
        .tx-sub { font-size: 0.65rem; }
        .tx-amount { font-size: 0.95rem; }
        .tx-value { font-size: 0.65rem; }
        
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.modern-row-marker) div[data-testid="stButton"] button { height: 34px !important; width: 34px !important; font-size: 0.95rem !important;}
    }
    </style>
    """, unsafe_allow_html=True)

    # 1. SLEEK ADD TRANSACTION BUTTON & FORM
    if not st.session_state.get('show_add_tx', False):
        st.markdown("<div class='btn-add-tx'></div>", unsafe_allow_html=True)
        ...
        with st.form("add_crypto", border=False):
            st.markdown("<div class='add-tx-card'></div><h3 style='text-align: center; color: white; margin-top: 0px; margin-bottom: 15px;'>New Transaction</h3>", unsafe_allow_html=True)
    ...
    st.markdown("<h4 style='color: white; margin-top: 10px; margin-bottom: 15px;'>Transaction History</h4>", unsafe_allow_html=True)
    
    # 2. SEAMLESS TRANSACTION LIST
    with st.container(height=600, border=False):
        for i, r in df_display.iterrows():
            ...
            sign = "+" if is_buy else "-"
            action_text = "Bought" if is_buy else "Sold"
            ...
            if st.session_state.get('confirm_delete_crypto') == orig_idx:
                with st.container(border=True):
                    ...
                    with c_yes:
                        if st.button("Delete", key=f"yes_del_{orig_idx}"):
                            ...
                            st.rerun()
                    with c_no:
                        if st.button("Cancel", key=f"no_del_{orig_idx}"):
                            st.session_state['confirm_delete_crypto'] = None
                            st.rerun()
            else:
                # Normal Modern Row
                with st.container(border=True):
                    st.markdown("<div class='modern-row-marker' style='display:none;'></div>", unsafe_allow_html=True)
                    
                    c_main, c_edit, c_del = st.columns([4, 0.5, 0.5])
                    with c_main:
                        st.markdown(f"""
                        <div class="modern-tx-row">
                            <div class="tx-left">
                                <img src="{get_ticker_logo(r['Ticker'])}" class="tx-logo" onerror="this.src='https://via.placeholder.com/40/1e2a44/ffffff?text={r['Ticker'][0]}';">
                                <div>
                                    <div class="tx-title">{r['Ticker']}</div>
                                    <div class="tx-sub">{format_datum(r['Datum'])} • {action_text} @ ${format_price(price)}</div>
                                </div>
                            </div>
                            <div class="tx-right">
                                <div class="tx-amount {'tx-buy' if is_buy else 'tx-sell'}">{sign}{format_holdings(abs_amount, r['Ticker'])}</div>
                                <div class="tx-value">{format_money(abs_usdc)}</div>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                    with c_edit:
                ...
                if st.session_state.get('edit_crypto_row') == orig_idx:
                    with st.form(f"edit_crypto_form_{orig_idx}", border=False):
                        st.markdown("<div class='edit-rollout'></div><h4 style='color: #00ff9d; margin-top: 0px; margin-bottom: 15px;'>✏️ Edit Transaction</h4>", unsafe_allow_html=True)
                        ...
                        with e_save: 
                            if st.form_submit_button("💾 Save"):
                                st.session_state.crypto_df.loc[orig_idx] = {
                                    "Datum": date_to_excel_serial(new_date), 
                                    "USDC": new_usdc if tx_type_edit == "Buy" else -new_usdc, 
                                    "Ticker": new_ticker, 
                                    "Amount": new_amount if tx_type_edit == "Buy" else -new_amount, 
