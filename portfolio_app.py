...
# ====================== CHART FUNCTION (fixed 5m/30m + more candles) ======================
...
def get_cryptocompare_ohlc(ticker: str, candle: str):
    ...
    if not sym:
        return None
    try:
        # Safe limit for minute data (still plenty of candles after resampling)
        if candle in ["5m", "30m"]:
        ...
        if coin_list:
            ...
            for i, coin in enumerate(coin_list):
                with selected_tab[i]:
                    ...
                    # DAILY OPEN PILL (smaller, no "24H" text, closer to LIVE)
                    ...
                    <div style="display:flex;gap:8px;margin-bottom:16px;">
                        ...
                        {f'<div style="background:#0f172a;padding:8px 16px;border-radius:9999px;display:inline-flex;align-items:center;gap:8px;margin-left:12px;"><span style="font-size:1.15rem;font-weight:700;color:#ffffff;">AVG</span><span style="font-size:1.45rem;font-weight:700;color:#ffaa00;">{format_crypto_price(avg_price)}</span></div>' if avg_price is not None else ''}
                    </div>
                    """, unsafe_allow_html=True)
