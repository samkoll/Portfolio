...
# The CSS hides this sidebar, but the Javascript clicks these buttons to trigger page loads
with st.sidebar:
...
if (!window.swipeListenerActive) {{
    ...
    // Finds and clicks the hidden Streamlit sidebar buttons based on their exact text
    window.parent.navTo = function(btnKey) {{
        const btns = Array.from(doc.querySelectorAll('[data-testid="stSidebar"] button p'));
        const target = btns.find(b => b.innerText === btnKey);
    ...
    doc.addEventListener('touchstart', e => {{
        ...
        const t = e.target;
        // Don't swipe if touching inside a chart or horizontal scroll area
...
if st.session_state.page == "Home":
    ...
    <head>
        ...
        <style>
            ...
            /* Responsive Expanded Chart Adaptation (NO FULLSCREEN) */
