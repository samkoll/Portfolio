...
if 'crypto_df' not in st.session_state: st.session_state.crypto_df = get_initial_crypto_df()
if 'fiat_df' not in st.session_state: st.session_state.fiat_df = get_initial_fiat_df()
...
# Kept safely in the DOM but hidden off-screen so Javascript can trigger transitions perfectly
with st.sidebar:
    st.markdown("<div id='st-nav-anchor'></div>", unsafe_allow_html=True)
...
st.markdown(f"""
<style>
@keyframes slideInFromRight {{ 0% {{ transform: translateX(30vw); opacity: 0; }} 100% {{ transform: translateX(0); opacity: 1; }} }}
@keyframes slideInFromLeft {{ 0% {{ transform: translateX(-30vw); opacity: 0; }} 100% {{ transform: translateX(0); opacity: 1; }} }}

/* HIDDEN NATIVE ROUTING: Sidebar is pushed out of view but remains accessible to the DOM engine */
...
<div id="page-metadata-anchor" data-current-page="{st.session_state.page}" style="display:none;"></div>
...
# Injected via the img onerror hack directly into the parent DOM.
js_engine = """
if (!window.swipeListenerActive) {
    window.swipeListenerActive = true;
    const doc = window.parent.document;
    
    window.parent.navTo = function(btnText) {
        const btns = Array.from(doc.querySelectorAll('[data-testid="stSidebar"] button p'));
        const target = btns.find(p => p.textContent && p.textContent.trim() === btnText);
    ...
    doc.addEventListener('touchend', e => {
        ...
        if (Math.abs(dX) > 60 && Math.abs(dX) > Math.abs(dY) * 1.5) {
            const anchor = doc.getElementById('page-metadata-anchor');
            if (!anchor) return;
            const current = anchor.getAttribute('data-current-page');
...
"""
components.html(f"<img src='dummy' style='display:none;' onerror='{js_engine}'>", height=0, width=0)
...
def build_portfolio_history(crypto_df, fiat_df, last_prices, hist_dict):
    ...
    for d in date_range:
        history_data.append({
            ...
            'value': safe_float(total_portfolio_value.loc[d]), 
            'invested': safe_float(cum_fiat_usdc.loc[d]), 
            'btc': safe_float(btc_benchmark_value.loc[d])
        })
...
if st.session_state.page == "Home":
    ...
    if history_data_raw:
        ...
        for idx, d in enumerate(history_data_raw):
            ...
            if idx == len(history_data_raw) - 1 and ts == today_ts: val, inv = safe_float(total_value), safe_float(fiat_usdc_total)
    ...
    pie_data_js = ",\n".join([f"{{ name: '{r['Ticker']}', y: {safe_float(r['Value'])}, color: '{get_ticker_color(r['Ticker'])}' }}" for _, r in df_port.iterrows() if r['Ticker'] != 'USDC' and pd.notna(r['Value']) and r['Value'] > 0])
    ...
    val_data_js = ",\n".join([f"{{ name: '{r['Ticker']}', y: {safe_float(r['Value']) if pd.notna(r['Value']) else 0}, color: '{get_ticker_color(r['Ticker'])}99' }}" for _, r in df_iv.iterrows()])
    inv_data_js = ",\n".join([f"{{ name: '{r['Ticker']}', y: {safe_float(r['USDC']) if pd.notna(r['USDC']) else 0.0}, color: '#64748b99' }}" for _, r in df_iv.iterrows()])
    ...
    # Javascript data variables isolated from CSS injection completely
    data_script = f"""
    <script>
        window.pnlDataMap = {{
        ...
        window.pieData = [{pie_data_js}];
        window.histValData = [{hist_val_js}];
        window.histBtcData = [{hist_btc_js}];
        window.histInvData = [{hist_inv_js}];
        window.invValCategories = {inv_val_categories_js};
        window.invData = [{inv_data_js}];
        window.valData = [{val_data_js}];
        window.roiData = [{roi_data_js}];
        window.dailyData = [{daily_data_js}];
        window.allocationData = [{allocation_series_js}];
    </script>
    """

    charts_html = data_script + """
    <!DOCTYPE html>
    <html>
    <head>
        <meta charset="UTF-8">
        ...
        <style>
            ...
            /* Native Flex Expand Method (from backup) */
            .chart-placeholder { width: 400px; flex: 0 0 400px; height: 340px; scroll-snap-align: center; transition: all 0.3s ease; }
            .chart-placeholder[data-type="pie"] { width: 350px; flex: 0 0 350px; }
            .chart-placeholder[data-type="history"] { width: 600px; flex: 0 0 600px; }
            .chart-placeholder[data-type="allocation"] { width: 600px; flex: 0 0 600px; }
            .chart-placeholder[data-type="inv-val"] { width: 500px; flex: 0 0 500px; }
            
            .chart-box { width: 100%; height: 100%; background: rgba(15, 23, 42, 0.4); border: 1px solid rgba(255,255,255,0.05); border-radius: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); touch-action: pan-x pan-y; will-change: transform; position: relative; display: flex; flex-direction: column; transition: all 0.3s ease; }
            ...
            /* Native Screen Width Expansion on Double Tap */
            .expanded-chart { width: 90vw !important; flex: 0 0 90vw !important; max-width: 100vw; }
            .expanded-chart .chart-box { background: rgba(15, 23, 42, 0.98) !important; border: 1px solid rgba(255, 255, 255, 0.15) !important; box-shadow: 0 15px 40px rgba(0,0,0,0.5) !important; z-index: 1001; }
        ...
        <script>
            ...
            try {
                Highcharts.stockChart('history-container', {
                    ...
                    credits: { enabled: false },
                    series: [
                        { name: 'Portfolio Value', data: window.histValData, color: '#00ff9d', fillColor: { linearGradient: { x1: 0, y1: 0, x2: 0, y2: 1 }, stops: [ [0, 'rgba(0, 255, 157, 0.5)'], [1, 'rgba(0, 255, 157, 0.0)'] ] }, zIndex: 3 }, 
                        { name: 'BTC Benchmark', type: 'line', data: window.histBtcData, color: '#f7931a', lineWidth: 2, zIndex: 2 }, 
            ...
            try {
                Highcharts.chart('inv-val-container', {
                    ...
                    xAxis: { type: 'category', categories: window.invValCategories, labels: { style: { color: '#94a3b8', fontWeight: 'bold', fontSize: '10px' } }, gridLineColor: 'rgba(255,255,255,0.05)', tickWidth: 0 },
            ...
            // Restored the PERFECT horizontal inline flex-expansion for Double Tap!
    ...
    """
    components.html(charts_html, height=365, scrolling=False)
    ...
    if not cards_html: cards_html = "<div style='color:#94a3b8; text-align:center; width:100%; margin-top: 40px;'>No assets to display. Add some transactions!</div>"
    ...
    <head>
        ...
        <style>
            * {{ box-sizing: border-box; }}
            ...
            .flip-card {{ flex: 0 0 420px; background-color: transparent; height: 320px; perspective: 1200px; cursor: pointer; scroll-snap-align: center; transition: all 0.3s ease; }}
            ...
            /* Native Screen Adaptation for Cards on Double Tap */
            .expanded-card {{ width: 90vw !important; flex: 0 0 90vw !important; max-width: 100vw; }}
            .expanded-card .flip-card-front, .expanded-card .flip-card-back {{ background: rgba(15, 23, 42, 0.98) !important; border: 1px solid rgba(255, 255, 255, 0.15) !important; box-shadow: 0 15px 40px rgba(0,0,0,0.5) !important; }}
    ...
    <script>
        ...
        flipCards.forEach(card => {{
            ...
            // Allow double-tap expansion for cards too!
            let lastTap = 0;
            card.addEventListener('touchend', function(e) {{
                const currentTime = new Date().getTime();
                const tapLength = currentTime - lastTap;
                if (tapLength < 400 && tapLength > 0 && window.innerWidth <= 768) {{
                    e.preventDefault(); e.stopPropagation();
                    if (card.classList.contains('expanded-card')) card.classList.remove('expanded-card');
                    else {{
                        document.querySelectorAll('.expanded-card').forEach(c => c.classList.remove('expanded-card'));
                        card.classList.add('expanded-card');
                        setTimeout(() => {{ card.scrollIntoView({{ behavior: 'smooth', block: 'nearest', inline: 'center' }}); }}, 50);
                    }}
                    setTimeout(() => {{ if (chartCache[ticker] && chartCache[ticker].chartObj) chartCache[ticker].chartObj.resize(); }}, 300);
                }}
                lastTap = currentTime;
            }});
        }});
...
# ================== PAGE 2: CRYPTO ==================
    ...
    df_display['USDC'] = pd.to_numeric(df_display['USDC'], errors='coerce').fillna(0.0)
