...
<div id="native-bottom-bar">
    <div class="nav-tab {home_active}" id="tab-home" onclick="Array.from(document.querySelectorAll('button p')).find(p => p.innerText.includes('ROUTER_HOME'))?.closest('button').click()">
        <span class="nav-icon">🏠</span>
        <span class="nav-lbl">Overview</span>
    </div>
    <div class="nav-tab {crypto_active}" id="tab-crypto" onclick="Array.from(document.querySelectorAll('button p')).find(p => p.innerText.includes('ROUTER_CRYPTO'))?.closest('button').click()">
        <span class="nav-icon">📊</span>
        <span class="nav-lbl">Crypto</span>
    </div>
    <div class="nav-tab {fiat_active}" id="tab-fiat" onclick="Array.from(document.querySelectorAll('button p')).find(p => p.innerText.includes('ROUTER_FIAT'))?.closest('button').click()">
        <span class="nav-icon">💰</span>
...
# --- 3. JS Swipe Controller & Router Hider ---
js_controller = f"""
<script>
(function() {{
    const parentDoc = window.parent.document;

    // Safely hide the routing buttons by text ONLY, leaving the rest of your app untouched
    Array.from(parentDoc.querySelectorAll('button p')).forEach(p => {{
        if (p.innerText.includes('ROUTER_')) {{
            const btn = p.closest('button');
            if (btn) btn.style.display = 'none';
            const wrapper = p.closest('div[data-testid="stButton"]');
    ...
    parentDoc.addEventListener('touchend', e => {{
        ...
        if (Math.abs(diffX) > Math.abs(diffY) && Math.abs(diffX) > 75) {{
            ...
            const clickTarget = (btnText) => {{
                const p = Array.from(parentDoc.querySelectorAll('button p')).find(p => p.innerText.includes(btnText));
                if (p) p.closest('button').click();
            }};
