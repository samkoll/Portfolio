...
# ====================== GLOBAL CSS (LIGHT + DARK THEME COMPATIBLE) ======================
st.markdown("""
<style>
    ...
    /* Glossy elements - work in both themes */
    ...
    [data-theme="light"] .glossy-box > div:first-child { color: #334155; }
    [data-theme="light"] .glossy-box > div:last-child { color: #0f172a; }
...
<div style="display:flex;gap:25px;margin-bottom:30px;flex-wrap:wrap;">
    ...
    <div class="glossy-box"><div>PnL %</div><div style="color:{'#00ff9d' if total_pnl_pct>=0 else '#ff4d4d'}">{"▲" if total_pnl_pct>0 else "▼" if total_pnl_pct<0 else ""} {abs(total_pnl_pct):.2f}%</div></div>
        ...
        html = f"""<html><head><style>
            ...
            thead th{{padding:12px 8px;text-align:center;font-size:0.95rem;color:inherit;}}
            ...
            [data-theme="light"] .row-inner{{background:#f8fafc;color:#0f172a;}}
        ...
        # ... (your original crypto page code remains exactly as you had it)
