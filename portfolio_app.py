...
# Prevents NaNs from corrupting Javascript data arrays and crashing charts
def safe_float(val):
    try:
        f = float(val)
...
with st.sidebar:
    nav_items = ["Home", "Crypto Transactions", "Fiat Transactions"]
...
if (!window.mySwipeEngineLoaded) {
    ...
    document.addEventListener('touchstart', e => {
        ...
        if (e.target.tagName === 'IFRAME') return; // Skip if touching iframe charts
    ...
    document.addEventListener('touchend', e => {
        ...
        if (Math.abs(dX) > 60 && Math.abs(dX) > Math.abs(dY)) {
            const pageDiv = document.getElementById('current-page-marker');
            const current = pageDiv ? pageDiv.getAttribute('data-page') : 'Home';
            
            if (dX > 0) { 
                if (current === 'Home') window.navTo('Nav_Crypto');
                else if (current === 'Crypto Transactions') window.navTo('Nav_Fiat');
            } else { 
                if (current === 'Fiat Transactions') window.navTo('Nav_Crypto');
                else if (current === 'Crypto Transactions') window.navTo('Nav_Home');
            }
        }
    }, {passive: true});
}
"""
components.html(f"<script>window.parent.eval({json.dumps(main_dom_js)});</script>", height=0, width=0)
...
st.markdown(f"""
<div id="current-page-marker" data-page="{st.session_state.page}" style="display:none;"></div>
<style>
...
if st.session_state.page == "Home":
    ...
    <head>
        ...
        <style>
            ...
            /* Responsive Expanded Chart Adaptation */
    ...
    <body>
        ...
        <script>
            ...
            // Allow PC users to scroll charts horizontally with mouse wheel
