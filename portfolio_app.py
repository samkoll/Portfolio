...
# ====================== GLOBAL CSS (Polished + Perfect Glow Overflow) ======================
st.markdown("""
<style>
/* ====================== GLOBAL LAYOUT ====================== */
.stApp {
...
/* === COIN CARDS - Premium Dark Style with Overflowing Glow === */
.coin-grid {
...
.coin-card:hover {
    ...
    box-shadow: 0 0 45px 18px var(--glow) !important;
    z-index: 30;
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(98px, 1fr)); gap: 14px; margin-bottom: 30px;">
    ...
    <div class="glossy-box"><div>PnL %</div><div style="color:{'#00ff9d' if total_pnl_pct>=0 else '#ff4d4d'}">{"▲" if total_pnl_pct>0 else "▼" if total_pnl_pct<0 else ""} {abs(total_pnl_pct):.2f}%</div></div>
        ...
        for _, r in df_port.iterrows():
            ...
            glow_color = '#00ffcc55' if base_color == '#000000' else base_color + '88'
            ticker = r['Ticker']
            ...
            cards_html += f"""
<div class="coin-card" data-glow="{glow_color}" {onclick}>
    <div class="card-header">
        <img src="{logo_url}" onerror="this.src='https://via.placeholder.com/42/1e2a44/ffffff?text={ticker[0]}';">
        <span>{ticker}</span>
    </div>
...
.coin-grid {{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:18px;padding:20px 12px;box-sizing:border-box;max-height:580px;overflow-y:auto;scrollbar-width:none;overflow:visible !important;}}
.coin-grid::-webkit-scrollbar {{display:none;}}
.coin-card {{background:#0f172a;padding:18px 20px;border-radius:20px;box-shadow:0 8px 25px rgba(0,0,0,0.45);transition:all 0.35s cubic-bezier(0.4,0,0.2,1);cursor:pointer;position:relative;z-index:1;outline:none;overflow:visible !important;border:1px solid rgba(255,255,255,0.07);}}
.coin-card:hover {{transform:translateY(-8px);box-shadow:0 0 45px 18px var(--glow) !important;z-index:30;border-color:rgba(0,255,200,0.3);}}
.card-header {{display:flex;align-items:center;margin-bottom:16px;gap:12px;}}
.card-header img {{width:42px;height:42px;border-radius:50%;object-fit:contain;}}
.card-header span {{font-weight:700;font-size:1.35rem;color:#ffffff;}}
.card-content {{display:flex;flex-direction:column;gap:9px;}}
.label-value-row {{display:flex;justify-content:space-between;align-items:center;font-size:0.96rem;}}
.label {{color:#aaaaaa;font-weight:500;}}
.value {{font-weight:600;color:#e0e0e0;}}
.total {{font-size:1.22rem;margin-top:10px;padding-top:10px;border-top:1px solid rgba(255,255,255,0.1);}}
.total-value {{font-size:1.32rem;font-weight:700;color:#ffffff;}}
...
<div id="price-charts-section" class="glossy-box" style="background:#1e2a44;padding:18px 20px;border-radius:18px;">
    <div style="display:flex;align-items:center;gap:12px;font-size:1.4rem;font-weight:700;">
        {CHARTS_ICON}
        <span>Charts</span>
    </div>
</div>
